...
internal class UpdateAnalyzer : IUpdateAnalyser
{
    ...
    public void OnUpdate()
    {
        lock (lockObj)
        {
            ...
            while (now - updateTimestamps.Peek() > WindowInterval) updateTimestamps.TryDequeue(out _);
    ...
    public DateTime EstimateNextUpdate()
    {
        lock (lockObj)
        {
            try
            {
                if (updateTimestamps.Count <= 1) return DateTime.MaxValue;

                if (cachedTime != DateTime.MaxValue) return cachedTime;
