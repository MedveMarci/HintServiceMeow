...
public class PlayerDisplay : IPlayerDisplay
{
    ...
    /// </summary>
    /// <remarks>
    ///     Setting a shorter interval may increase update frequency, potentially impacting
    ///     performance.
    /// </remarks>
    ...
    public void RemoveHint(IEnumerable<AbstractHint>? hints)
    {
        ...
        string groupName = Assembly.GetCallingAssembly().FullName;
        foreach (AbstractHint hint in hints) InternalRemoveHint(groupName, hint);
    }
    ...
    internal void ShowCompatibilityHint(string assemblyName, string? content, float duration, IParameter[]? parameters = null)
    {
        adapter.ShowHint(new CompatibilityAdaptorArg(assemblyName, content, duration, parameters));
    }
    ...
    private IEnumerator<float> CoroutineMethod()
    {
        while (true)
        {
            ...
            if (!isSuccessful) yield return 1f;
        }
    }
    ...
    private void OnResolutionUpdate(object sender, PropertyChangedEventArgs ev)
    {
        if (sender is not IScreenResolution)
            return;
