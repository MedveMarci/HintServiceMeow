...
public sealed class LineIndentTag : RichTag
{
    ...
    /// <inheritdoc />
    public override string OpenTag => $"<line-indent={value}>";
    ...
    /// </param>
    /// <returns>A new <see cref="LineIndentTag" /> with the specified indent value.</returns>
    public static LineIndentTag Get(string value)
    {
        return new LineIndentTag(value);
    }

    public static LineIndentTag Get(int pixels)
    {
        return new LineIndentTag(pixels.ToString());
    }
}