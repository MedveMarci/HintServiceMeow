...
| EdgeMargin | `float` | Насколько далеко (в единицах) подсказка с выравниванием по левому/правому краю удерживается от края экрана, когда `ResolutionOption` равен `Offset`. `0` (по умолчанию) отправляет подсказку вплотную к краю; положительное значение сдвигает её внутрь. |
...
// Держите подсказку немного дальше от края, а не вплотную к нему
var insetHint = new Hint
{
    Text = "Не совсем у края",
