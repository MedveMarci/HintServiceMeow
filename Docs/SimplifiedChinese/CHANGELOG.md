...
- 添加了 `AbstractHint::EdgeMargin`，用于让左/右对齐的提示与屏幕边缘保持可配置的距离，而不是紧贴边缘。
...
#### 性能优化
- 缓存了富文本解析结果，使未变化的提示可直接从缓存读取，而不必在每个更新周期都重新完整解析。
- 减少了解析热路径中 `FontTool`、`Tokenizer` 和 `RichTextParser::HandleText` 的逐字符开销。
- 将 `TaskScheduler` 的多次加锁属性读取合并为单次快照读取，降低了轮询开销。
- 通过使用不可变的输出快照，减少了 `PlayerDisplay` 每次更新的内存分配，并修复了显示输出的遍历竞争问题。
- 当渲染内容未发生变化时跳过冗余的提示网络发送（携带动画/提示参数的提示始终会发送）。
- 在没有可见的 Dynamic Hint 时跳过动态提示碰撞区域的计算。
- 为热路径上的调试日志字符串添加了守卫，使其仅在启用调试日志时才会构建。

#### 依赖项
- 将 `Northwood.LabAPI` 更新至 `1.1.7` 版本。
...
### 修复
#### 兼容性适配器
- 修复了兼容性提示无法解析 `{0}`、`{1}` 等参数的问题。
- 修复了兼容性提示被错误地归属于 LabAPI/游戏本体而非实际发送该提示的插件，导致 `DisabledCompatAssemblies` 无法针对单个插件生效的问题。
- 修复了兼容性提示的提示效果（淡入淡出、脉冲等）会波及共享同一渲染提示的其他插件内容的问题；此类效果现在会被丢弃而不会被应用。
- 修复了当兼容性提示的持续时间为无限或非法值（NaN）时，移除该提示会导致崩溃的问题。
- 修复了过宽的兼容性提示行不会按单词边界换行、而是相互重叠的问题。
- 修复了多行兼容性提示渲染时没有自然行间距的问题。
- 修复了兼容性提示在没有显式 size 标签时使用了错误字体大小的问题。
- 修复了兼容性提示会被屏幕分辨率边缘偏移影响的问题；它们现在使用 `ResolutionOption.None`。

#### 渲染
- 修复了在 `ResolutionOption.Offset` 下右对齐提示无法到达真实屏幕边缘的问题（此前它们只能到达固定画布边缘，而左对齐提示则不受此限制）。
- 修复了相对 `<size>` 值（百分比/em）是基于提示自身字体大小而非固定参考值来计算，从而可能压缩行间距的问题。
- 修复了空行的高度会坍缩为零、而不是使用当前字体大小的问题。

#### 稳定性
- 修复了当解析任务在 `PlayerDisplay` 已被销毁后才完成时，`TaskScheduler` 会抛出 `ObjectDisposedException` 的问题。
